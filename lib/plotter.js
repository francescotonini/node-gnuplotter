...
function setup_gnuplot(gnuplot, options) {
	if (options.format === 'svg') { /* Setup gnuplot for SVG */
		gnuplot.stdin.write(`set term svg fname "${options.font || 'Helvetica'}" fsize ${options.fontSize || 13}\n`);
	...
	if (_.isArray(options.lineStyle)) {
		for (var i = 0; i < options.lineStyle.length; i++) {
			var current = options.lineStyle[i];
			gnuplot.stdin.write(`set style ${options.style || 'line'} ${(i + 1)} lw ${current.weight || '1'} lc rgb "${current.color || 'red'}"\n`);
		}

		gnuplot.stdin.write('set style increment user\n');
	}
